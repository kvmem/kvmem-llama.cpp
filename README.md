...
> **Bonsai experimental branch (`0.16.0-rc3-prism.3`):** Windows CUDA 12.9.86 builds include **RTX 20/30/40/50** targets (SM75/86/89/120a). Hardware validation is limited to RTX 5050 Laptop and 5060 Ti. The launcher defaults to **MTP off**, original PTQ1 weights, 128K context, 24K retrieval + 10K generation pool, Q8 target KV, and a 4096-token thinking budget. Optional `-Mtp` uses F16 draft KV; add `-DraftTokens 2` to try the measured code-generation improvement. See the [new version notes](docs/milestones/v0.16.0-rc3-prism.3.md), [Bonsai quick start](scripts/windows/README-bonsai.md), and [coding measurements](docs/bonsai-coding-mtp012-5060.md). Long-context recall is still experimental; the configured 128K limit is not a quality guarantee.
...
**Release status:** [Download prism.3 — Experimental Pre-release](https://github.com/kvmem/kvmem-llama.cpp/releases/tag/v0.16.0-rc3-prism.3), publicly available with runtime ZIP, matching source ZIP, validation and SHA256 files. This experimental release is tagged from the Bonsai branch and does not replace the stable release or merge Bonsai into master. The rc3 benchmarks and IQ3/IQ4 recipes below describe the previous baseline, not Bonsai.
...
Latest published milestone: [`v0.16.0-rc2`](docs/milestones/v0.16.0-rc2.md) (pre-release).
...
Source development version: **0.16.0-rc3** (not yet released). See the [rc3 change notes](docs/milestones/v0.16.0-rc3.md). The download links above still point to published packages.
...
The source version of `llama-kvmem-server` accepts the common flags below with
their llama.cpp meanings. These additions require a rebuild; the published rc2
binary predates them. This is an independent, single-slot server, so it does not
