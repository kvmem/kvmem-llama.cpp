...
ggml_type kvmem_parse_cache_type(const char * s, bool * ok) {
    ...
    // TurboQuant KV codecs (CUDA). Only the wired-and-verified set is accepted here.
    if (std::strcmp(s, "turbo3_tcq") == 0) { return GGML_TYPE_TURBO3_TCQ; }
    if (std::strcmp(s, "turbo2_tcq") == 0) { return GGML_TYPE_TURBO2_TCQ; }
    if (std::strcmp(s, "turbo4")     == 0 || std::strcmp(s, "turbo4_0") == 0) { return GGML_TYPE_TURBO4_0; }
    if (std::strcmp(s, "turbo8")     == 0 || std::strcmp(s, "turbo8_0") == 0) { return GGML_TYPE_TURBO8_0; }
    if (ok) {
        *ok = false;
    }
    return GGML_TYPE_F16;
