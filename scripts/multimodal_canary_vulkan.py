#!/usr/bin/env python3
"""Vulkan/AMD variant of multimodal_canary.py (NVML-free). Same CLI and artifacts."""
import argparse
import base64
import csv
import json
...
class VulkanSampler(threading.Thread):
    """Device-wide VRAM samples; logged allocations are estimates, not samples."""

    VRAMMON = os.environ.get('KVMEM_VRAMMON', '/root/vrammon')
    BUF_RE = re.compile(r'(model buffer size|KV buffer size|RS buffer size|compute buffer size)\s*=\s*([\d.]+) MiB')
    ...
    def __init__(self, folder):
        ...
        self.rows = []
        self.errors = []
        self.static_used_mib = None
        self.measurement_source = None
        configured = os.environ.get('KVMEM_DRM_DEVICE')
        devices = [Path(configured)] if configured else list(Path('/sys/class/drm').glob('card[0-9]*/device'))
        devices = [p for p in devices if (p / 'mem_info_vram_used').is_file()]
        self.drm_device = devices[0] if len(devices) == 1 else None

    def _probe_vrammon(self):
        if self.drm_device:
            try:
                total = int((self.drm_device / 'mem_info_vram_total').read_text()) / 1048576
                used = int((self.drm_device / 'mem_info_vram_used').read_text()) / 1048576
                self.measurement_source = str(self.drm_device)
                return total, used
            except (OSError, ValueError) as exc:
                if len(self.errors) < 20:
                    self.errors.append(f'DRM: {exc}')
        if not os.access(self.VRAMMON, os.X_OK):
            return None
        try:
            out = subprocess.check_output([self.VRAMMON], text=True, timeout=10)
            m = re.search(r'total=(\d+)MiB budget=(\d+)MiB used=(\d+)MiB', out)
            if m:
                self.measurement_source = self.VRAMMON
                return int(m.group(1)), int(m.group(3))
        except Exception as exc:
            if len(self.errors) < 20:
                self.errors.append(f'vrammon: {exc}')
        return None

    def _probe_static(self, log_text):
        used = 0.0
        found = False
        for _, size in self.BUF_RE.findall(log_text):
            used += float(size)
            found = True
        if found:
            self.static_used_mib = used
        return self.static_used_mib
        ...
        with (self.folder / 'vram.csv').open('w') as fh:
            ...
            while not self.stop_event.is_set():
                with (self.folder / 'server.stderr.log').open() as log:
                    ...
                    tail = log.read()
                    offset = log.tell()
                if self.phase == 'prefill' and 'KVMEM_CHAT_PREFILL ' in tail:
                    self.phase = 'decode'
                probe = self._probe_vrammon()
                if probe is not None:
                    total, used = probe
                else:
                    with (self.folder / 'server.stderr.log').open() as log:
                        self._probe_static(log.read())
                    total, used = None, None
                free = max(total - used, 0) if total is not None else None
                row = (time.monotonic() - start, self.phase, used, free, None, None, None, None)
    ...
    def finish(self):
        ...
        for phase in sorted({r[1] for r in self.rows}):
            rows = [r for r in self.rows if r[1] == phase and r[2] is not None]
            phases[phase] = {'peak_mib': max((r[2] for r in rows), default=None),
                             'last_mib': rows[-1][2] if rows else None,
                             'min_free_mib': min((r[3] for r in rows), default=None), 'samples': len(rows)}
        return {'peak_vram_mib': max((r[2] for r in self.rows if r[2] is not None), default=None),
                'vram_metric_scope': 'device-wide',
                'vram_measurement_source': self.measurement_source,
                'vram_sample_count': sum(r[2] is not None for r in self.rows),
                'logged_allocation_estimate_mib': self.static_used_mib,
...
def main():
    ...
    ap.add_argument('--binary', type=Path, default=ROOT / 'build-vulkan/bin/llama-kvmem-server')
    ...
    env.update(LD_LIBRARY_PATH=os.pathsep.join(filter(None, [str(args.binary.resolve().parent), env.get('LD_LIBRARY_PATH')])),
               NO_PROXY='127.0.0.1,localhost', no_proxy='127.0.0.1,localhost')
    # Full canaries assert trace events; upstream now disables them by default.
    trace_enabled = args.trace or not args.quick or args.performance
    if trace_enabled:
        env['KVMEM_TRACE'] = '1'
    ...
    try:
        ...
        stats.update(options=vars(args), requests=results, malloc_arena_max=env['MALLOC_ARENA_MAX'],
                     trace_enabled=env.get('KVMEM_TRACE') == '1')
